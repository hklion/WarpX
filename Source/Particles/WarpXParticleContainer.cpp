                                        ...
                                        amrex::Real const dt, amrex::Real const relative_time)
{
    ...
    const Real ppc_avg = static_cast<Real>(np_to_depose)/static_cast<Real>(tilebox.numPts());
    ...
    if (WarpX::current_deposition_algo == CurrentDepositionAlgo::Esirkepov) {
        if        (WarpX::nox == 1){
            doEsirkepovDepositionShapeN<1>(
                ...
                jx_arr, jy_arr, jz_arr, np_to_depose, ppc_avg, dt, relative_time, dx, xyzmin, lo, q,
        ...
        if        (WarpX::nox == 1){
            doVayDepositionShapeN<1>(
                GetPosition, wp.dataPtr() + offset, uxp.dataPtr() + offset,
                uyp.dataPtr() + offset, uzp.dataPtr() + offset, ion_lev,
                jx_fab, jy_fab, jz_fab, np_to_depose, ppc_avg, dt, relative_time, dx, xyzmin, lo, q,
                WarpX::n_rz_azimuthal_modes, cost,
                WarpX::load_balance_costs_update_algo);
        } else if (WarpX::nox == 2){
